...
def calculate_gross_photosynthesis(
    par_ppfd: float, co2_ppm: float, p_max: float = 30.0, alpha: float = 0.045
) -> float:
    """
    Calculates gross photosynthetic rate (umol CO2/m2/s) using a non-rectangular 
    hyperbola light response curve modulated by CO2 concentration.
    """
...
def calculate_vpd(temp_c: float, rh_pct: float) -> float:
    ...
    return max(0.01, vpsat - vpack)


def calculate_canopy_transpiration(vpd_kpa: float, dli_mol_m2_day: float) -> float:
    """Estimates daily canopy transpiration (L/m2/day) based on VPD and light load."""
